...
export default function CreateCockpitDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => { if (!next) onClose(); }}>
      <DialogContent className="flex h-[min(82vh,680px)] flex-col gap-0 overflow-hidden border border-app-border bg-app-surface-elevated p-0 text-app-text sm:!max-w-2xl">
          ...
          <div className="space-y-4 pb-1">
            ...
            <div>
              ...
              <textarea
                ...
                className="min-h-[136px] w-full resize-none rounded-3xl border border-app-border-subtle bg-app-surface px-4 py-3.5 text-sm leading-relaxed text-app-text-secondary outline-none transition-colors placeholder:text-app-text-subtle focus:border-red-500/25"
                disabled={executing}
              />
            </div>
            ...
            {mode === 'template' && templates && templates.length > 0 && (
              <div className="rounded-[24px] border border-app-border-subtle bg-app-surface-subtle/35 p-3.5">
                <div className="flex items-center justify-between gap-3">
                  <div>
                    <div className="text-sm font-medium text-app-text-secondary">选择模板</div>
                ...
                <div className="mt-3 grid max-h-40 gap-2 overflow-y-auto pr-1 md:grid-cols-2">
                    ...
                    return (
                      ...
                      >
                        ...
                        {template.description && (
                          <div className="mt-2 line-clamp-2 text-[11px] leading-5 text-app-text-subtle">
                            {template.description}
                          </div>
                        )}
                      </button>
