...
function CardShell({
  ...
  return (
    <div className="mt-3 rounded-xl border border-white/[0.08] bg-white/[0.03] overflow-hidden animate-in slide-in-from-bottom-3 fade-in duration-500">
      <div className="px-5 py-3.5 border-b border-white/[0.05] bg-white/[0.02]">
        <div className="flex items-center justify-between">
          <div>
            ...
            {card.subtitle && (
              <p className="text-xs text-white/30 mt-0.5">{card.subtitle}</p>
            )}
          </div>
        </div>
      </div>
      ...
      {card.actions && card.actions.length > 0 && (
        <div className="px-5 py-3 border-t border-white/[0.05] bg-white/[0.01] flex flex-wrap gap-2">
              ...
              className={`text-xs h-8 ${
                action.variant === 'primary'
                  ...
                  : action.variant === 'secondary'
                    ? 'bg-white/[0.06] hover:bg-white/[0.1] border-white/[0.08] text-white/70'
                    : action.variant === 'danger'
                      ? ''
      ...
      {card.metric && (
        <div className="flex items-end gap-3 mb-5">
          <div>
            ...
            <div className="text-xs text-white/30 mt-0.5">{card.metric.label}</div>
          </div>
          {card.metric.change && (
            <Badge
              variant="outline"
        ...
        <div className="space-y-3">
          {card.chartData.map((item, i) => (
            <div key={i} className="flex items-center gap-3">
              ...
              <span className="text-xs font-medium text-white/50 w-16 text-right shrink-0">
                ¥{item.value}万
              </span>
            </div>
          ))}
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-white/[0.06]">
              {cols.map((col: TableColumn) => (
                <th
                  key={col.key}
                  className="text-left py-2 px-2 font-medium text-white/30 whitespace-nowrap"
          ...
          <tbody>
            {rows.map((row, i) => (
              ...
              >
                {cols.map((col: TableColumn) => (
                  <td key={col.key} className="py-2.5 px-2 text-white/60 whitespace-nowrap">
          ...
          <div key={i} className={field.type === 'textarea' ? 'sm:col-span-2' : ''}>
            ...
            {field.type === 'select' ? (
              <Select defaultValue={field.value}>
                <SelectTrigger className="h-9 text-xs bg-white/[0.03] border-white/[0.08] text-white/70">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-[#1a1a2e] border-white/[0.08]">
                  {field.options?.map((opt) => (
                    <SelectItem key={opt} value={opt} className="text-xs text-white/70">
                      {opt}
                    </SelectItem>
                  ))}
                </SelectContent>
              ...
              <Textarea
                ...
                className="text-xs min-h-[80px] resize-none bg-white/[0.03] border-white/[0.08] text-white/70 placeholder:text-white/20"
              />
            ) : field.type === 'date' ? (
              <Input
                type="date"
                defaultValue={field.value}
                className="h-9 text-xs bg-white/[0.03] border-white/[0.08] text-white/70"
              />
            ) : (
              <Input
                type={field.type}
                defaultValue={field.value}
                placeholder={field.placeholder}
                className="h-9 text-xs bg-white/[0.03] border-white/[0.08] text-white/70 placeholder:text-white/20"
              />
            )}
          </div>
        ))}
          ...
          return (
            <div key={i} className="relative">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] text-white/20 w-12 shrink-0">
              ...
              {i < steps.length - 1 && (
                <div
                  className={`w-0.5 flex-1 my-1 ${
                    step.status === 'completed' ? 'bg-emerald-500/30' : 'bg-white/[0.05]'
                  }`}
                />
              )}
            </div>
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`text-sm font-medium ${
                    step.status === 'pending' ? 'text-white/25' : 'text-white/80'
                  }`}
                >
                  {step.label}
                </span>
              ...
              {step.description && (
                <p
                  className={`text-xs mt-0.5 ${
                    step.status === 'pending' ? 'text-white/15' : 'text-white/30'
                  }`}
                >
                  {step.description}
                </p>
...
function ApprovalCard({ card }: { card: CardData }) {
  ...
  return (
    <CardShell card={card}>
            ...
            <div key={key} className="p-3 rounded-lg bg-white/[0.02] border border-white/[0.04]">
              ...
              <div
                className={`text-sm font-semibold ${
                  key === 'amount' ? 'text-white text-lg' : 'text-white/60'
                }`}
              >
                {value}
              </div>
      ...
      <div className="mt-4 pt-3 border-t border-white/[0.05] flex items-center gap-2 text-xs text-white/25">
        ...
        {data &&
          Object.entries(data).map(([key, value]) => (
            ...
            >
              ...
              <ExternalLink className="w-3.5 h-3.5 text-white/10 ml-auto opacity-0 group-hover:opacity-100 transition-opacity" />
            </div>
          ))}
      </div>
    </CardShell>
