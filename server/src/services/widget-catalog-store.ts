...
const BUILTIN_WIDGETS = [
  ...
  {
    id: 'builtin-business-message-center',
    name: '消息中心',
    type: 'business',
    category: '业务组件',
    icon: 'Bell',
    color: '#8b5cf6',
    description: '统一承载审批、待办、通知和预警消息，重点支持智能审批列表、风险分析和快捷处理动作。',
    agentDescription: '当用户需要统一处理审批、待办、通知、预警或跨系统消息入口时，优先选择 business 消息中心组件。该组件可由 YonClaw skill、OpenAPI、CLI 或本地连接器提供数据，并支持审批动作执行。',
    useCases: ['审批中心', '待办中心', '风险预警消息', '统一通知入口'],
    tags: ['business', 'approval', 'message', 'actionable'],
    schemaHint: {
      recommendedDataShape: {
        businessType: 'message-center',
        messages: 'Array<{ id,type,priority,status,title,summary,source,dueAt,intelligence,actions }>',
      },
      layoutAdvice: '建议 w=6、h=5；审批优先放在首屏右侧或顶部，适合 actionable 模式。',
      styleConfig: {
        interactionMode: 'actionable',
        connectorPolicy: 'yonclaw | openapi | cli | local',
      },
    },
    template: {
      type: 'business',
  ...
  {
    id: 'builtin-business-calendar',
    name: '智能日程',
    type: 'business',
    category: '业务组件',
    icon: 'CalendarDays',
    color: '#0ea5e9',
    description: '聚合个人、团队和驾驶舱相关日程，支持会议、审批截止、风险提醒、里程碑和快速添加交互。',
    agentDescription: '当用户需要展示日程、会议、截止时间、审批时限、业务提醒或全局时间轴时，优先选择 business 智能日程组件。该组件可连接 YonClaw skill、企业日历 OpenAPI 或本地 CLI。',
    useCases: ['经营例会', '审批截止', '风险提醒', '项目里程碑'],
    tags: ['business', 'calendar', 'schedule', 'timeline'],
    schemaHint: {
      recommendedDataShape: {
        businessType: 'calendar',
        events: 'Array<{ id,title,type,start,end,location,participants,source,status,actions }>',
      },
      layoutAdvice: '建议 w=5-6、h=5；适合与消息中心联动，审批截止和风险事项可自动形成日程。',
      styleConfig: {
    ...
    template: {
      ...
      business: {
        category: 'business',
        businessType: 'calendar',
        dataContract: 'calendar-events.v1',
        actionContract: 'calendar-actions.v1',
        connectorPolicy: { preferred: 'openapi', fallback: ['yonclaw', 'local'] },
  ...
  {
    id: 'builtin-business-insight-hub',
    name: '业务洞察',
    type: 'business',
    category: '业务组件',
    icon: 'Lightbulb',
    color: '#d97706',
    description: '驾驶舱顶部的智能摘要面板。聚合当前驾驶舱的核心数据、用户角色与关注维度，生成关键指标、风险/机会洞察和可执行建议。',
    agentDescription: '当用户需要为驾驶舱创建顶部综合概览、跨组件生成经营洞察、风险识别、机会建议或行动推荐时，优先选择 business 业务洞察组件（businessType=insight-hub）。该组件是驾驶舱的智能摘要面板，不是驾驶舱本身，需包含 cockpitSummary（组件身份）、persona（用户画像）、highlights（核心指标）、insights（关键洞察）和 recommendations（全局建议）。',
    useCases: ['驾驶舱顶部概览', '经营洞察', '风险识别', '机会建议', '跨组件联动分析'],
    tags: ['business', 'insight', 'context', 'recommendation', 'summary'],
    schemaHint: {
      recommendedDataShape: {
        businessType: 'insight-hub',
        cockpitSummary: '{ title: "xx业务洞察", subtitle?, domain?, scope?, description? }',
        persona: '{ role: string, focus: string[], preferences?: string[] }',
        highlights: 'Array<{ id, label, value, change?, trend: "up"|"down"|"neutral" }>',
        insights: 'Array<{ id, title, type: "risk"|"opportunity"|"anomaly"|"recommendation"|"summary", severity, summary, evidence?, recommendation?, confidence?, actions? }>',
        recommendations: 'Array<{ id, text, priority: "high"|"medium"|"low" }>',
      },
      layoutAdvice: '建议 w=12、h=4，放在驾驶舱顶部作为通栏概览；可与消息中心、日程组件联动。',
      styleConfig: {
        interactionMode: 'agent-assisted',
    ...
    template: {
      ...
      business: {
        category: 'business',
        businessType: 'insight-hub',
        dataContract: 'business-insights.v2',
        actionContract: 'insight-actions.v1',
        connectorPolicy: { preferred: 'yonclaw', fallback: ['openapi', 'local'] },
        permissions: ['insight.read', 'insight.generate'],
        interactionMode: 'agent-assisted',
      },
      data: {
        businessType: 'insight-hub',
        cockpitSummary: { title: '业务洞察', description: '驾驶舱智能摘要面板' },
        persona: { role: '业务负责人', focus: ['核心指标', '关键风险', '行动建议'] },
        highlights: [],
        insights: [],
        recommendations: [],
      },
