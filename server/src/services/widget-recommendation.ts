...
const BASE_SIZE: Record<string, WidgetSize> = {
  metric: { w: 3, h: 2 },
  chart: { w: 6, h: 4 },
  table: { w: 6, h: 4 },
  kanban: { w: 6, h: 4 },
...
export function recommendWidgetType(rawType: unknown, rawData: unknown): string {
  const type = typeof rawType === 'string' ? rawType.trim() : '';
  const data = asRecord(rawData);
  const inferred = inferWidgetType(data);

  if (type && !isTypeMismatched(type, data)) return type;
  if (inferred && inferred !== 'universal') return inferred;
  if (type) return type;
  return inferred || 'universal';
}
...
export function recommendWidgetSize(type: string, rawData?: unknown): WidgetSize {
  const data = asRecord(rawData);
  const base = BASE_SIZE[type] || BASE_SIZE.universal;
